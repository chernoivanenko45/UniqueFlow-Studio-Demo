...
async function serveFile(request, env, token) {
  ...
  const object = await env.BETA_FILES.get(env.BETA_OBJECT_KEY, {
  ...
  if (!request.headers.has("range") || request.headers.get("range")?.startsWith("bytes=0-")) {
    const day = new Date().toISOString().slice(0, 10);
    const countKey = `downloads:${day}`;
    const previous = Number(await env.BETA_LINKS.get(countKey) || 0);
    await env.BETA_LINKS.put(countKey, String(previous + 1), { expirationTtl: 90 * 24 * 60 * 60 });
  }
